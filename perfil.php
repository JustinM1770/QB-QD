...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['actualizar_perfil'])) {
    try {
        ...
        if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
            throw new Exception('Error de seguridad: Token CSRF inválido. Por favor recarga la página.');
        }
...
<head>
    ...
    <link rel="stylesheet" href="assets/css/app-native.css">
   <style>
    :root {
        --primary: #0165FF;
    ...
    * {
    ...
    /* Ocultar card de referidos en otras pestañas */
    #card-referidos {
        display: block;
    }

    .section:not(#perfil).active #card-referidos {
...
<div class="membership-status <?php echo $esMiembroActivo ? 'active' : 'inactive'; ?>">
    ...
    <div class="card" id="card-referidos">
        ...
        <?php if ($datosReferidos['total_referidos'] > 0): ?>
            ...
            <h6>Progreso para obtener pedido + envío gratis:</h6>
                        
                    <!-- Opción 1: Por referidos -->
            <div class="progress-option mb-3 p-3 border rounded">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    <span><strong>Opción 1:</strong> Refiere 2 usuarios que compren 2 productos o membresía</span>
                    <?php if ($datosReferidos['referidos_que_compraron'] >= 2): ?>
                ...
                <div class="progress">
                    <div class="progress-bar" role="progressbar" 
                        style="width: <?php echo min(100, ($datosReferidos['referidos_que_compraron'] / 2) * 100); ?>%">
                        <?php echo $datosReferidos['referidos_que_compraron']; ?>/2
                    </div>
                </div>
            </div>

            <!-- Opción 2: Por compras propias -->
            <div class="progress-option p-3 border rounded">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    <span><strong>Opción 2:</strong> Realiza 2 compras tú mismo</span>
                    <?php if ($datosReferidos['compras_realizadas'] >= 2): ?>
                        <span class="badge bg-success">¡Completado!</span>
                    <?php endif; ?>
                </div>
                <div class="progress">
                    <div class="progress-bar bg-info" role="progressbar" 
                         style="width: <?php echo min(100, ($datosReferidos['compras_realizadas'] / 2) * 100); ?>%">
                        <?php echo $datosReferidos['compras_realizadas']; ?>/2
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <div class="alert alert-success">
                            ...
                            <strong>¡Felicidades!</strong> Has desbloqueado tu beneficio gratuito.
                            <br>
                            <small>Incluye: Pedido completo + Envío gratis (hasta $300 MXN)</small>
                            <br>
                            <button class="btn btn-success btn-sm mt-2" onclick="usarBeneficioReferido()">
                                <i class="fas fa-gift me-1"></i>Activar beneficio ahora
                            </button>
                        </div>
                    <?php elseif ($datosReferidos['beneficio_referido_usado']): ?>
                        <div class="alert alert-info">
                            <i class="fas fa-check-circle me-2"></i>
                            Ya has utilizado tu beneficio gratuito. ¡Gracias por usar QuickBite!
                        </div>
                    <?php else: ?>
...
function cambiarSeccion(seccion) {
    ...
    // OCULTAR REFERIDOS EN OTRAS PESTAÑAS
    const cardReferidos = document.getElementById('card-referidos');
    if (cardReferidos) {
        if (seccion === 'perfil') {
            cardReferidos.style.display = 'block';
        } else {
            cardReferidos.style.display = 'none';
        }
    }
    ...
    switch(seccion) {
        ...
        function toggleReferidosVisibility(seccion) {
    const cardReferidos = document.querySelector('.card:has(.fa-users)') || 
                         document.querySelector('.card .fa-users')?.closest('.card');
    ...
    if (cardReferidos) {
        ...
        function guardarDireccion() {
            ...
            // Mapear campo predeterminada al nombre que espera el API
            if (formData.get('predeterminada') === 'on') {
                formData.set('es_predeterminada', '1');
            }
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // Recargar direcciones desde el servidor
                    cargarDirecciones();
                } else {
                    mostrarNotificacion(data.error || 'Error al guardar dirección', 'danger');
                }
            })
            .catch(err => {
                console.error('Error guardando dirección:', err);
                mostrarNotificacion('Error de conexión al guardar dirección', 'danger');
            });
        }
            ...
            if (confirm('¿Estás seguro de eliminar esta dirección?')) {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        mostrarNotificacion(data.message || 'Error al eliminar dirección', 'danger');
                    }
                })
                .catch(err => {
                    console.error('Error eliminando dirección:', err);
                    mostrarNotificacion('Error de conexión al eliminar dirección', 'danger');
                });
            }
        }
